function D1_Realignment_all_runs(sj_dir, runs, filt)
...
for r = 1:numel(runs)
    if exist([sj_dir filesep runs{r}])==7
        run_dir = fullfile(sj_dir, runs{r});
        % select the files
        ...
        for i=1:size(V,1)
            files{i,1} = [repmat([run_dir filesep], numVols, 1) f repmat([',' int2str(i)] , numVols, 1)];
        end
        fileset{r}=files;
        clear f V files;
    end
...
if ~isempty(fileset)
    ...
    matlabbatch{1}.spatial{1}.realign{1}.estwrite.roptions.prefix    = 'r2';
