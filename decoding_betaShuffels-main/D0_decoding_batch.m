...
labelnames = {'StimPress','StimFlutt'; ...
    'StimPress','StimVibro'; ...
    'StimFlutt','StimVibro'; ...
    'ImagPress','ImagFlutt'; ...
    'ImagPress','ImagVibro'; ...
...
cd('C:\Users\saraw\Desktop\Masks\Null_CONJ_CONJ\005');
roi_names = dir(['*.nii']);

% SET OTHER DECODING CFG PARAMETERS in D4_Decoding_batch.m

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%# step 5: Decoding, cross classification
%'SET THE DIRECTORY WHERE YOUR SPM.MAT AND ALL RELATED BETAS ARE, individual 1st level glm directory';
betaDir5 = '1st_level_D0_wragf4d';
%'SPECIFY WHERE THE RESULTING ACCURACY MAPS SHOULD BE STORED';
outputDir5 = 'D5_XClass_wragf4d';
...
labelnames_train = {'StimPress','StimFlutt'; ...
    'StimPress','StimVibro'; ...
    'StimFlutt','StimVibro';...
    'ImagPress','ImagFlutt'; ...
    'ImagPress','ImagVibro'; ...
...
labelnames_test  = {'ImagPress','ImagFlutt'; ...
    'ImagPress','ImagVibro';...
    'ImagFlutt','ImagVibro'; ...
    'StimPress','StimFlutt'; ...
    'StimPress','StimVibro'; ...
...
vox_size=[2 2 2]; % voxel size in mm
smoo5 = 5; %smooth with kernel in mm
            ...
            for sj = sbjs %1:numel(SJs)
                try
                    for Lp=1:size(labelnames,1)
                        ...
                        output_path  = fullfile(src_dir, SJs{sj}, [outputDir4 '_' cell2mat(labelnames(Lp,1)) '_' cell2mat(labelnames(Lp,2))]); %% vlt mal drüber nachdenken, noch mehr unterordner zu machen
                        
                        D4_Decoding_batch(beta_path, output_path, labelnames(Lp,:), roi_names) %% the actual job
                        ...
                        if norm4
                            ...
                            if smoo4
                                ...
                                if ~ismember(Lp, excep_array)
                                    I4ostt{Lp}=fullfile([outputDir4 '_' cell2mat(labelnames(Lp,1)) '_' cell2mat(labelnames(Lp,2))],['s' num2str(smoo4) 'wres_accuracy_minus_chance.nii']);
                                end
                            else
                                if ~ismember(Lp, excep_array)
                ...
                for sj = sbjs %1:numel(SJs)
                    try
                        for Lp=1:size(labelnames_train,1)
                            ...
                            display(['Training: ' cell2mat(labelnames_train(Lp,1)) ', ' cell2mat(labelnames_train(Lp,2))])
                            display(['Test: ' cell2mat(labelnames_test(Lp,1)) ', ' cell2mat(labelnames_test(Lp,2))])
                            ...
                            if norm5
                                ...
                                if smoo5
                                    Images2{Lp}=fullfile(output_path,'wres_accuracy_minus_chance.nii');
                                    I4ostt{Lp}=fullfile([outputDir5 '_TRAIN_' cell2mat(labelnames_train(Lp,1)) '_' cell2mat(labelnames_train(Lp,2)) '_TEST_' cell2mat(labelnames_test(Lp,1)) '_' cell2mat(labelnames_test(Lp,2))],['s' num2str(smoo5) 'wres_accuracy_minus_chance.nii']);
                                else
                                    I4ostt{Lp}=fullfile([outputDir5 '_TRAIN_' cell2mat(labelnames_train(Lp,1)) '_' cell2mat(labelnames_train(Lp,2)) '_TEST_' cell2mat(labelnames_test(Lp,1)) '_' cell2mat(labelnames_test(Lp,2))],'wres_accuracy_minus_chance.nii');
                                end
                            elseif smoo5
                                for roi = 1:length(roi_names)
                                    Images2=[Images2; fullfile(output_path, strcat('res_accuracy_minus_chance_', roi_names(roi)))];
                                    I4ostt=[I4ostt; fullfile(strcat(outputDir5, ['_TRAIN_' cell2mat(labelnames_train(Lp,1)) '_' cell2mat(labelnames_train(Lp,2)) '_TEST_' cell2mat(labelnames_test(Lp,1)) '_' cell2mat(labelnames_test(Lp,2))], 's', num2str(smoo5), 'wres_accuracy_minus_chance_', roi_names(roi)))];
                                end
    %                             Images2{Lp}=fullfile(output_path,'res_accuracy_minus_chance.nii');
                            end
                        end
                ...
                if ostt5
                    ...
                    SJin(exclSJs5)=[];
                    for t=1:size(I4ostt,2)
                        D6_glm_2ndLevel_OneSampleTTest(src_dir, SJin, I4ostt{t}, explicit_mask5)
                    end
                end
                clear I4ostt SJin
