...
sbjs = [1 2 6 7 8 10 12 13 14 16 18 19 20 21 22 24 26 27 29 30 32]; %21
...
for s = sbjs %1:length(SJs)
    ...
    if ~isempty(runs)
        for r=1:length(runs)
            ...
            % cond 8: preCue
            onsets{r,9} = (log_ExPra19.Design(1,(log_ExPra19.Design(3,:)>0))/1000)-1;
            
            % cond 9: motion regressor
            onsets{r,10} = (log_ExPra19.Design(1,(log_ExPra19.Design(3,:)>0) & (log_ExPra19.responses(1,:)>0))/1000)+4;
            %onset der Reaktion per Knopfdruck: beginn trial + prep-time(1s) +
            %stim+time(3s) + answer-time(2s) -> zu ENDE der answer-time???

            % cond 11: fucked up imag
            onsets{r,11} = (log_ExPra19.Design(1,(log_ExPra19.Design(3,:)==2) & (log_ExPra19.responses(1,:)<2))/1000);
            ...
            tempxlsname = [outputdir filesep 'Onsets_goodImag_' SJs{s} '_' temprun '.xls'];
...
%             delete(tempxlsname);
        ...
        cd(outputdir)
        eval(['save all_onsets_goodImag_' SJs{s} '.mat onsets condnames'])
